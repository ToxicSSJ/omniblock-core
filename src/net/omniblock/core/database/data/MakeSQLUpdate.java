...
public class MakeSQLUpdate {
	...
	public void execute() throws SQLException, IllegalArgumentException {
		if(operation == TableOperation.INSERT || operation == TableOperation.UPDATE){
			if(rows.isEmpty()){
				throw new IllegalArgumentException("Una sentencia debe tener al menos una instruccion para realizar la operacion.");
			}
		}
		if(operation == TableOperation.UPDATE){
			if(where.isEmpty()){
				throw new IllegalArgumentException("La operacion UPDATE debe tener al menos una instruccion del tipo WHERE");
			}
		}
