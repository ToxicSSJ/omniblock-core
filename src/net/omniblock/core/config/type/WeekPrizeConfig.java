package net.omniblock.core.config.type;

import java.io.File;
import java.io.IOException;
import java.io.InputStream;
import java.net.URISyntaxException;

import org.bukkit.configuration.file.FileConfiguration;
import org.bukkit.configuration.file.YamlConfiguration;

import net.omniblock.core.config.Config;

public class WeekPrizeConfig implements Config {

	protected FileConfiguration bundle;
	protected File file;
	
	protected String filename;
	
	@Override
	public WeekPrizeConfig create(String filename) throws URISyntaxException, IOException {
		
		this.filename = filename;
		
		File file = new File(Config.getJARDirectory() + "/config/" + filename);
		
		if(!file.exists()){
			
			InputStream stream = WeekPrizeConfig.class.getResourceAsStream("/config/" + filename);
			Config.copyFile(stream, file);
			
		} 
		
		this.bundle = YamlConfiguration.loadConfiguration(file);
		this.file = file;
		
		return this;
		
	}
	
	@Override
	public void save() {
		try { bundle.save(file);
		} catch (IOException e) { e.printStackTrace(); }
		return;
	}
	
	@Override
	public File getFile() {
		return file;
	}
	
	@Override
	public FileConfiguration getConfiguration() {
		return bundle;
	}
	
}
